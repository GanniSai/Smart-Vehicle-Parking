...
if (isset($_POST['update_reservation'])) {
    ...
    if (isset($_POST['parking_end_date']) && !empty($_POST['parking_end_date'])) {
        $parking_duration = mysqli_real_escape_string($mysqli, trim($_POST['parking_end_date']));
    } else {
        $error = 1;
    ...
    $query = 'UPDATE reservations SET  code =?, client_name =?, client_phone =?, car_regno=?, lot_number =?, parking_date =?, parking_end_date =?, amt =? WHERE id = ?';
